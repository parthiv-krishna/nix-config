...
let
  reverseProxyCfg = config.custom.reverse-proxy;
  cfg = config.custom.selfhosted.crowdsec;
...
in
{
  ...
  custom.selfhosted.crowdsec = {
    enable = true;
    hostName = "nimbus";
    public = false;
    protected = false;
    port = 9090;
    config = lib.mkIf (reverseProxyCfg.enable && reverseProxyCfg.publicFacing) {
      ...
      system.activationScripts.crowdsec-plugins = {
        text = ''
          mkdir -p ${pluginDir};
          cp -f ${crowdsecPlugin}/bin/${pluginName} ${pluginDir}/${pluginName}
      ...
      services.crowdsec = {
        ...
        settings = {
          ...
          prometheus = {
            enabled = true;
            listen_port = config.custom.selfhosted.prometheus-crowdsec.port;
          };
          config_paths = {
        ...
        acquisitions = [
          ...
          # monitor Authelia logs
          {
            labels.type = "syslog";
            source = "journalctl";
            journalctl_filter = [ "_SYSTEMD_UNIT=authelia-${reverseProxyCfg.autheliaInstanceName}.service" ];
          }
        ];
      };
  ...
  custom.selfhosted.prometheus-crowdsec = {
    enable = true;
    hostName = "nimbus";
    public = false;
    protected = false;
